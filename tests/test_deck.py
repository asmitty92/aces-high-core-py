...
def test_deck_contains_all_clubs(standard_deck):
    for rank in Rank:
        assert Card(Suit.CLUBS, rank) in standard_deck.cards
        

def test_deck_contains_all_hearts(standard_deck):
    for rank in Rank:
        assert Card(Suit.HEARTS, rank) in standard_deck.cards
        

def test_deck_contains_all_spades(standard_deck):
    for rank in Rank:
        assert Card(Suit.SPADES, rank) in standard_deck.cards
        

def test_deck_contains_all_diamonds(standard_deck):
    for rank in Rank:
        assert Card(Suit.DIAMONDS, rank) in standard_deck.cards


def test_deck_shuffles_itself(standard_deck):
    original_deck = [card for card in standard_deck.cards]
    standard_deck.shuffle()
    assert original_deck != standard_deck.cards
...
def test_shuffle_moves_most_cards(standard_deck):
    original_deck = standard_deck.cards.copy()
    standard_deck.shuffle()
    # Count how many cards remain in the same position
    unchanged = sum(1 for o, s in zip(original_deck, standard_deck.cards) if o == s)
...
def test_shuffle_preserves_all_cards(standard_deck):
    original_set = set(standard_deck.cards)
    standard_deck.shuffle()
    assert set(standard_deck.cards) == original_set
